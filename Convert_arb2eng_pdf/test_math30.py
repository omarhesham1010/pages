...
text = "1 - small = large"  # Extracted right-to-left "الكبير = الصغير - 1"
...
def fix_math(t):
    ...
    if has_math_word and has_math_symbol:
        ...
        if t_new.strip().startswith('='):
            ...
            # THE FIX: If the output turns out exactly "Large = Small - [numbers]" 
            # Or "Large = Small + [numbers]", the user implicitly wants it to look like:
            # `= Large - Small - [numbers]`
            # Let's just catch "Large = Small" at the start of the string!
            
            # Match ^Large = Small (with optional spaces)
            reversed_t = re.sub(r'^(?i)Large\s*=\s*Small', '= Large - Small', reversed_t)
            
            # Wait, what if it's "1 - Small = Large" BEFORE reverse?
            # It reverses to "Large = Small - 1"
            # So `^Large = Small` matches "Large = Small", then we replace with `= Large - Small`
            # The `- 1` is left untouched.
            # Output: `= Large - Small - 1` 
